...
class TMM_Helper {    
    ...
    public static function get_cnsocial_buttons_list() {
        return array(
    ...
    public static function display_share_buttons($style = '', $post_id, $buttons = array(), $place = '', $options = array()) {
        ...
        if (count($buttons)==0){
         	$cn=(TMM::get_option("blog_single_show_cnsocial_share") == '1') ? self::get_cnsocial_buttons_list():array();       	
        	$us=(TMM::get_option("blog_single_show_social_share") == '1') ? self::get_social_buttons_list():array();
        ...
        foreach ($buttons as $key => $button) {
            ...
            switch ($key) {
            	  case 'weixin':
                    ...
                    $classes[] = 'weixin';
                    $share_title = _x('分享至微信', 'share buttons', 'diplomat');
                    $url = 'javascript:void(0);';
                    $custom = '  onclick="js_method()" class="jiathis_button_weixin" ';
                    break;
            	  case 'weibo':

                    $classes[] = 'weibo';
                    $share_title = _x('分享至新浪微博', 'share buttons', 'diplomat');
                    $url = add_query_arg(array('url' =>esc_url( $u), 'title' =>urlencode($t)), $protocol . '://v.t.sina.com.cn/share/share.php?appkey=3036462609');
                    break;
            	  case 'renren':

                    $classes[] = 'renren';
                    $share_title = _x('分享至人人网', 'share buttons', 'diplomat');
                    $url = add_query_arg(array('resourceUrl' =>esc_url( $u),'srcUrl' =>esc_url( $u), 'title' =>urlencode($t)), $protocol . '://widget.renren.com/dialog/share');
                    break;
            	  case 'tweibo':

                    $classes[] = 'tweibo';
                    $share_title = _x('分享至腾讯微博', 'share buttons', 'diplomat');
                    $url = add_query_arg(array('url' => esc_url($u), 'title' =>urlencode($t)), $protocol . '://share.v.t.qq.com/index.php?c=share&amp;a=index&amp;appkey=801497376');
                    break;
            	  case 'qqzone':

                    $classes[] = 'qqzone';
                    $share_title = _x('分享至QQ空间', 'share buttons', 'diplomat');
                    $url = add_query_arg(array('url' =>esc_url( $u), 'title' =>urlencode($t)), $protocol . '://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey');
                    break;
                case 'twitter':
            ...
            $url = ( $key ==  'weixin' )? $url: esc_url($url);

	        if($url){

		        $share_button = sprintf(
			        '<li class="%1$s"><a href="%2$s" rel="external nofollow" target="_blank" title="%4$s"%5$s>%3$s</a></li>', $classes_str, $url, $desc, $share_title, $custom
		        );
...
//Custom page navigation
			    ...
			    if (is_object($post)) {
				    ...
				    if (!empty($categories)) {
					    ...
					    $breadcrumbs[] = array(
						    ...
						    'title' => esc_attr(sprintf(__("查看 %s 中的全部文章", 'diplomat'), $categories->name)),
					    );
				    } else {
					    $breadcrumbs[] = array(
    ...
    public static function get_the_category_list($separator = '', $parents = '', $post_id = false) {
        ...
        if (empty($categories))
            return apply_filters('the_category', __('未分类', 'diplomat'), $separator, $parents);
        ...
        foreach ($categories as $category) {
            ...
            if ($cat == $category->term_id) {
                ...
                $thelist .= '<a href="' . esc_url(get_category_link($category->term_id)) . '" title="' . esc_attr(sprintf(__("查看 %s 中的全部文章", 'diplomat'), $category->name)) . '" ' . $rel . '>' . $category->name . '</a></li>';
            }
        }
