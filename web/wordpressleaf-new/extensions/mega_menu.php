...
class TMM_Menu_Walker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $icon_type_array = array(
            'icon-paper-plane-2' => __('类型', 'diplomat') . ': ' . 'icon-paper-plane-2',
            'icon-pencil-7' => __('类型', 'diplomat') . ': ' . 'icon-pencil-7',
            'icon-beaker-1' => __('类型', 'diplomat') . ': ' . 'icon-beaker-1',
            'icon-megaphone-3' => __('类型', 'diplomat') . ': ' . 'icon-megaphone-3',
            'icon-cog-6' => __('类型', 'diplomat') . ': ' . 'icon-cog-6',
            'icon-lightbulb-3' => __('类型', 'diplomat') . ': ' . 'icon-lightbulb-3',
            'icon-comment-6' => __('类型', 'diplomat') . ': ' . 'icon-comment-6',
            'icon-thumbs-up-5' => __('类型', 'diplomat') . ': ' . 'icon-thumbs-up-5',
            'icon-laptop' => __('类型', 'diplomat') . ': ' . 'icon-laptop',
            'icon-search' => __('类型', 'diplomat') . ': ' . 'icon-search',
            'icon-wrench' => __('类型', 'diplomat') . ': ' . 'icon-wrench',
            'icon-leaf' => __('类型', 'diplomat') . ': ' . 'icon-leaf',
            'icon-cogs' => __('类型', 'diplomat') . ': ' . 'icon-cogs',
            'icon-group' => __('类型', 'diplomat') . ': ' . 'icon-group',
            'icon-folder-close' => __('类型', 'diplomat') . ': ' . 'icon-folder-close',
            'icon-cloud' => __('类型', 'diplomat') . ': ' . 'icon-cloud',
            'icon-briefcase' => __('类型', 'diplomat') . ': ' . 'icon-briefcase',
            'icon-beaker' => __('类型', 'diplomat') . ': ' . 'icon-beaker',
            'icon-bullhorn' => __('类型', 'diplomat') . ': ' . 'icon-bullhorn',
            'icon-comment' => __('类型', 'diplomat') . ': ' . 'icon-comment',
            'icon-globe' => __('类型', 'diplomat') . ': ' . 'icon-globe',
            'icon-globe-6' => __('类型', 'diplomat') . ': ' . 'icon-globe-6',
            'icon-heart' => __('类型', 'diplomat') . ': ' . 'icon-heart',
            'icon-rocket' => __('类型', 'diplomat') . ': ' . 'icon-rocket',
            'icon-suitcase' => __('类型', 'diplomat') . ': ' . 'icon-suitcase',
            'icon-pencil' => __('类型', 'diplomat') . ': ' . 'icon-pencil',
            'icon-params' => __('类型', 'diplomat') . ': ' . 'icon-params',
            'icon-folder-open' => __('类型', 'diplomat') . ': ' . 'icon-folder-open',
            'icon-cog' => __('类型', 'diplomat') . ': ' . 'icon-cog',
            'icon-coffee' => __('类型', 'diplomat') . ': ' . 'icon-coffee',
            'icon-gift' => __('类型', 'diplomat') . ': ' . 'icon-gift',
            'icon-home' => __('类型', 'diplomat') . ': ' . 'icon-home',
            'icon-lightbulb' => __('类型', 'diplomat') . ': ' . 'icon-lightbulb',
            'icon-thumbs-up' => __('类型', 'diplomat') . ': ' . 'icon-thumbs-up',
            'icon-umbrella' => __('类型', 'diplomat') . ': ' . 'icon-umbrella',
            'icon-th-list' => __('类型', 'diplomat') . ': ' . 'icon-th-list',
            'icon-resize-small' => __('类型', 'diplomat') . ': ' . 'icon-resize-small',
            'icon-download-alt' => __('类型', 'diplomat') . ': ' . 'icon-download-alt'
        );
        ...
        if (!empty($item->_invalid)) {
            ...
            $title = sprintf(__('%s (无效)', 'diplomat'), $item->title);
        } elseif (isset($item->post_status) && 'draft' == $item->post_status) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(__('%s (待定)', 'diplomat'), $item->title);
        }
    ...
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes); ?>" data-parent="<?php echo $item->menu_item_parent ?>">
        <dl class="menu-item-bar">
            <dt class="menu-item-handle">
                <span class="item-title"><?php echo esc_html($title); ?> <i <?php echo $style_ititle ?>><?php _e(' (列) ', 'diplomat'); ?></i></span>
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('上移', 'diplomat'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('下移', 'diplomat'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('编辑菜单项', 'diplomat'); ?>" href="<?php
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo $item_id; ?>">
                    <?php _e('导航标签', 'diplomat'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr($item->title); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
                    <?php _e('标题属性', 'diplomat'); ?><br />
                ...
                <label for="edit-menu-item-description-<?php echo $item_id; ?>">
                    <?php _e('描述', 'diplomat'); ?><br />
                        <textarea id="edit-menu-item-description-<?php echo $item_id; ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id; ?>]"><?php
                            ...
                            // textarea_escaped
                            ?></textarea>
                    <span class="description"><?php _e('如果当前主题支持，那么该描述将显示在菜单中。', 'diplomat'); ?></span>
                </label>
            </p>
            ...
            <div class="advanced_menu_options">
                ...
                <div class="set_image_menu" <?php echo $style_image; ?>>
                    ...
                    <?php if (!empty($menu_item_image) && ($menu_item_image!='none')) { ?>
                        <a  data-id="<?php echo esc_attr($item_id); ?>" href="#" class="remove_menu_item_image"><?php _e('移除特色图像', 'diplomat'); ?></a>
                    <?php } else { ?>
                        <a  data-id="<?php echo esc_attr($item_id); ?>" href="#" class="add_menu_item_image"><?php _e('添加特色图像', 'diplomat'); ?></a>
                    <?php } ?>
                ...
                <div class="set_shortcode_menu">
                    ...
                    <a href="#" data-id="<?php echo esc_attr($editor_id); ?>" class="show_editor" ><?php _e('显示高级编辑器', 'diplomat'); ?></a>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if ('custom' != $item->type && $original_title !== false){ ?>
                    <p class="link-to-original">
                        <?php printf(__('源： %s', 'diplomat'), '<a href="' . esc_url($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
                    </p>
                <?php } ?>
                ...
                <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url(add_query_arg(array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg($removed_args, admin_url('nav-menus.php')))); ?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('取消', 'diplomat'); ?></a>
            </div>
    ...
    static function nav_menu_meta_box() {
        ...
        add_meta_box('tmm_navmenu_editor_metabox', __('自定义框体', 'diplomat'), array(__CLASS__, 'editor_menu_custom_box'), 'nav-menus', 'side', 'default');

    }
    static function editor_menu_custom_box() {
