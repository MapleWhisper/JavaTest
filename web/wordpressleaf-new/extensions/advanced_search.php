...
class TMM_Advanced_Search {
    ...
    public static function advanced_search() {
        ...
        $s = trim($_REQUEST['s']);  
        //$encode = mb_detect_encoding($s);
        //$s = mb_convert_encoding($s, 'ASCII', $encode);
        ...
        if (TMM::get_option('search_title')){
            ...
            //$parts[] = "($wpdb->posts.post_title REGEXP '$words')";
        ...
        $search_messages = array(
            'no_criteria_matched' => esc_html__("什么都没有找到", 'diplomat'),
            'another_search_term' => esc_html__("请尝试其他搜索条件（注意中文输入法请敲两下空格键结束）", 'diplomat'),
            'time_format' => get_option('date_format'),
            // 'all_results_link'    => http_build_query($serch_args),
            'view_all_results' => esc_html__('查看全部结果', 'diplomat')
        );
