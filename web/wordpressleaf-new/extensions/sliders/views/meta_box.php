...
<div class="slider">
	...
	<div class="native_sliders_groups" <?php if ($page_slider_type == 'layerslider' OR $page_slider_type == 'cuteslider'): ?>style="display: none;"<?php endif; ?>>
		<p><?php esc_html_e("幻灯片组", 'diplomat') ?>:</p>
		<?php if (!empty($slides)): ?>
			<div class="sel"><select name="page_slider">
					<option value=""><?php esc_html_e("选择幻灯片组", 'diplomat') ?></option>
		...
		<?php else: ?>
			<p><?php esc_html_e("您还没有创建任何幻灯片组。", 'diplomat') ?><br><a href="<?php echo esc_url(admin_url('post-new.php?post_type=' . TMM_Slider::$slug)); ?>"><?php esc_attr_e("在此点击", 'diplomat') ?></a> <?php esc_attr_e("去创建一个", 'diplomat') ?></p>
		<?php endif; ?>
	</div>
	...
	<?php if (is_plugin_active('LayerSlider/layerslider.php') || class_exists('LayerSlider')): ?>
		<div id="layerslider_slidegroups" <?php if ($page_slider_type != 'layerslider'): ?>style="display: none;"<?php endif; ?>>
			...
			?>
			<p><?php esc_html_e("Layerslider的组", 'diplomat') ?>:</p>
			<div class="sel">
				<select name="layerslider_slide_group">
					...
					<?php if (!empty($sliders)) : ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_html_e("你还没有为你的layerslider创建任何幻灯片组。", 'diplomat') ?>
					<?php endif; ?>
				</select>
			</div>
		</div>
	...
	<?php if (function_exists('cuteslider_init')): ?>
		<div id="cutelider_slidegroups" <?php if ($page_slider_type != 'cuteslider'): ?>style="display: none;"<?php endif; ?>>
			...
			$sliders = $wpdb->get_results("SELECT * FROM $table_name
											...
											ORDER BY id ASC LIMIT 200");
			?>
			<p><?php esc_attr_e("Cuteslider的组", 'diplomat') ?>:</p>
			<div class="sel">
				<select name="cuteslider_slide_group">
					<option value=""><?php esc_attr_e("选择幻灯片组", 'diplomat') ?></option>
					<?php if (!empty($sliders)) : ?>
						<?php foreach ($sliders as $item) : ?>
							<?php $name = empty($item->name) ? 'Unnamed' : $item->name; ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_attr_e("你还没有为你的cuteslider创建任何幻灯片组。", 'diplomat') ?>
					<?php endif; ?>
				</select>
			</div>
		</div>
