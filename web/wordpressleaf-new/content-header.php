	...
	<div class="large-12 columns">
		<div class="page-title">
			...
			<?php else:
				...
				<?php if (is_search()): ?>
					<h1><?php printf(__('搜索结果： %s', 'diplomat'), '<span>' . get_search_query() . '</span>'); ?></h1>
				<?php endif; ?>
					...
					<?php
						...
						<?php if (is_object($queried_object)): ?>
							<?php if (@$queried_object->taxonomy == 'category'):$is_defined = true; ?>
								<h1><?php printf(__('分类： %s', 'diplomat'), '<span>' . $queried_object->name . '</span>'); ?></h1>
							<?php endif; ?>
						<?php endif; ?>
						...
						<?php if ($is_defined == false): ?>
							<h1>
								<?php if (is_day()) : ?>
									<?php printf(__('每日文章： %s', 'diplomat'), '<span>' . get_the_date() . '</span>'); ?>

								<?php elseif (is_month()) : ?>
									<?php printf(__('每月文章： %s', 'diplomat'), '<span>' . get_the_date(_x('F Y', 'monthly archives date format', 'diplomat')) . '</span>'); ?>

								<?php elseif (is_year()) : ?>
									<?php printf(__('每年文章： %s', 'diplomat'), '<span>' . get_the_date(_x('Y', 'yearly archives date format', 'diplomat')) . '</span>'); ?>

								<?php elseif (is_author()) : ?>
									<?php printf(__('作者文章： %s', 'diplomat'), '<span>' . get_the_author_meta( 'display_name' ) . '</span>'); ?>

								<?php elseif (is_tag()): ?>
									<?php printf(__('标签文章： %s', 'diplomat'), '<span>' . single_tag_title('', false) . '</span>'); ?>

								<?php elseif (is_post_type_archive('forum')): ?>
									<?php _e('论坛', 'diplomat'); ?>

								<?php else : ?>
									<?php _e('博客文章', 'diplomat'); ?>
								<?php endif; ?>
							</h1>
