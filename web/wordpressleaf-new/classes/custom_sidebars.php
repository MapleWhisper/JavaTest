...
class TMM_Custom_Sidebars {
	...
	public static function get_categories_select($selected = '', $name = '', $id = '') {
		...
		$args = array(
			'show_option_none' => __('选择分类', 'diplomat'),
			'hide_empty' => 0,
			'echo' => 0,
			'selected' => $selected,
			'hierarchical' => 0,
		...
		if(empty($html)){
			$html = '<select><option>'. __('没有分类', 'diplomat') .'</option></select>';
		}
	...
	public static function get_pages_select($selected = '', $name = '', $id = '') {
		$args = array(
			'show_option_none' => __('选择页面', 'diplomat'),
			'selected' => $selected,
			'echo' => 0,
			'name' => $name,
			'id' => $id
		...
		if ($show_default_sidebar) {
			if (function_exists('dynamic_sidebar') AND dynamic_sidebar('tmm_default_sidebar')):else:
				...
				<div class="widget widget_categories">
					<h3 class="widget-title"><?php esc_html_e('分类', 'diplomat') ?></h3>
					<ul class="categories">
				...
				<div class="widget widget_calendar">
					<h3 class="widget-title"><?php esc_html_e('日历', 'diplomat') ?></h3>
					<?php get_calendar(); ?>
				</div>

				<div class="widget widget_meta">
					<h3 class="widget-title"><?php esc_html_e('管理', 'diplomat') ?></h3>
					<ul>
						<?php wp_register(); ?>
						<li>
							<?php wp_loginout(); ?>
