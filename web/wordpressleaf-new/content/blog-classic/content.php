		...
		<div class="entry-content">
			...
			if( strpos( $post->post_content, '<!--more-->' ) ){
				...
				if (empty($post->post_excerpt)) {
					...
					$txt = strip_tags($txt);
					$txt = (mb_strlen($txt)>$excerpt_symbols_count ) ? (mb_substr($txt, 0, $excerpt_symbols_count) . " ...") : $txt;

				} else {
					$txt =  (mb_strlen($post->post_excerpt) > $excerpt_symbols_count) ? (mb_substr($post->post_excerpt, 0, $excerpt_symbols_count) . " ...") : $post->post_excerpt;
				}
				echo '<p>'.$txt.'</p>';
			}
			?>
