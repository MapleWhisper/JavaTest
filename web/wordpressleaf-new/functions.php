...
define('TMM_THEME_NAME', 'WordPressLeaf');
define('TMM_FRAMEWORK_VERSION', '2.1.3');
define('TMM_SITE_LINK', 'http://webtemplatemasters.com/');
define('TMM_THEME_LINK', 'http://www.wordpressleaf.com/');
define('TMM_THEME_FORUM_LINK', 'http://www.wordpressleaf.com/');
...
if ( !function_exists('tmm_get_social_types') ) {
	function tmm_get_social_types() {
		return array(
			...
			'weixin' => array(
				'name' => '微信',
				'link' => 'http://www.wordpressleaf.com/wp-content/themes/wordpressleaf/assets/images/weixin.png',
			),
			'weibo' => array(
				'name' => '新浪微博',
				'link' => 'http://weibo.com',
		  ...
		  'qqzone' => array(
				'name' => 'QQ空间',
				'link' => 'http://user.qzone.qq.com/1794674215',
			),			
			'twitter' => array(
				'name' => 'Twitter',
	...
	function tmm_theme_first_activation() {
		...
		if (is_admin() && 'themes.php' == $pagenow && isset($_GET['activated'])) {
			...
			if (!$theme_was_activated) {
				...
				TMM::update_option('copyright_text', '版权所有 &copy; '.date('Y').'. <a target="_blank" href="http://www.wordpressleaf.com">'.__('WordPress Leaf', 'diplomat').'</a>. '.__('保留所有权利', 'diplomat'));
			}
...
if ( !function_exists('tmm_comments_form_defaults') ) {
	function tmm_comments_form_defaults($defaults) {
		...
		$defaults['fields']['author'] = '<div class="row"><div class="large-7 columns"><fieldset class="comment-form-author">' .
			'<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' placeholder="' . __('你的名字', 'diplomat') . ' *" /></fieldset>';
		$defaults['fields']['email'] = '<fieldset class="comment-form-email">' .
			'<input id="email" name="email" type="email" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . $aria_req . ' placeholder="' . __('你的电子邮箱', 'diplomat') . ' *" /></fieldset>';
		$defaults['fields']['url'] = '<fieldset class="comment-form-url"> ' .
			'<input id="url" name="url" type="url" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" placeholder="' . __('网站', 'diplomat') . '" /></fieldset></div></div>';
		$defaults['comment_field'] = '<fieldset class="comment-form-comment">' .
			'<textarea cols="30" rows="10" required="" id="comment" name="comment" placeholder="' . __('你的留言', 'diplomat') . ' *"></textarea></fieldset>';

		$defaults['comment_notes_before'] = '';
		$defaults['comment_notes_after'] = '';

		$defaults['cancel_reply_link'] = ' - ' . __('取消回复', 'diplomat');

		$defaults['title_reply'] = __('留下一条回复', 'diplomat');

		$defaults['label_submit'] = __('提交', 'diplomat');
                ...
                <article>
                    ...
                    <div class="comment-body">
                        ...
                        <div class="comment-meta">
                            ...
                            <div class="comment-date"><?php comment_date(); ?> <?php _e('在', 'diplomat'); ?> <?php comment_date('H:i'); ?></div>
                            <?php echo get_comment_reply_link(array_merge(
                                    array('reply_text' => __('回复', 'diplomat')),
...
if ( !function_exists('tmm_admin_menu') ) {
	function tmm_admin_menu() {
	    add_theme_page(__("春叶选项", 'diplomat'), __("春叶选项", 'diplomat'), 'manage_options', 'tmm_theme_options', 'tmm_theme_options_page');
	}
}
...
if ( !function_exists('tmm_register_scripts') ) {
    ...
    wp_register_script('jiathis', 'http://v3.jiathis.com/code/jia.js?uid=1', array('jquery'), '', true);
    wp_register_script( 'google-prettify', TMM_THEME_URI . '/js/google-code-prettify/prettify.js?', array('jquery'), '20170324', true);
    wp_register_script( 'sticky-sidebar', TMM_THEME_URI . '/js/theia-sticky-sidebar.js?', array('jquery'), '20170331', true);
    wp_register_script( 'lowweixin', TMM_THEME_URI . '/js/lowweixin.js?', array('jquery'), '20170411', true);
	}
}
...
if ( !function_exists('tmm_register_styles') ) {
	function tmm_register_styles() {
		...
		wp_register_style( 'google-prettify', TMM_THEME_URI .'/js/google-code-prettify/prettify.css', null, '20170324' );
...
if ( !function_exists('tmm_wp_enqueue_scripts') ) {
	function tmm_wp_enqueue_scripts() {
		/* Head scripts */
    wp_enqueue_style( 'leaf-fontawesome', '//netdna.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.css' );
		...
		/*加载国内社交媒体-微信*/
		$blog_single_show_cnsocial_share = TMM::get_option("blog_single_show_cnsocial_share"); if ($blog_single_show_cnsocial_share == '1' && is_singular() && (!is_home() && !is_front_page())) { 
		wp_enqueue_script('jiathis');
	  }
	  /*加载谷歌代码高显*/
	...
	function output_wp_editor_widget_html() {
		...
		<div id="tmm-editor-widget-container"  class="" style="display: none;">
			...
			<div class="tmm-popup-header">
				<h3 class="tmm-popup-title"><?php esc_attr_e( '小工具编辑器', 'diplomat' ); ?></h3>
	...
	function tmm_get_plugins() {
		...
		return array(
			...
			*/
			...
			array(
				'name'               => TMM_THEME_NAME.' Features',
				'slug'               => 'tmm_theme_features',
				'source'             => get_stylesheet_directory() . '/lib/plugins/tmm_theme_features.zip',
				'required'           => true,
			...
			/*array(
				...
				'slug'               => 'tmm_db_migrate',
				'source'             => 'https://github.com/ThemeMakers/tmm_db_migrate/archive/diplomat_v2.0.3.zip',
				'required'           => false,
				'version'           => '2.0.3',
			),*/
			array(
				...
				'source'             => get_stylesheet_directory() . '/lib/plugins/tmm_content_composer.zip',
				'required'           => true,
				'version'           => '1.0.7',
			),
			/*array(
				...
				'source'             => 'https://github.com/ThemeMakers/tmm_events_calendar/archive/diplomat_v1.0.6.zip',
				'required'           => false,
				'version'           => '1.0.6',
			),*/
			/*array(
				...
				'source'             => 'https://downloads.wordpress.org/plugin/seamless-donations.4.0.14.zip',
				'required'           => false,
				'version'           => '4.0.14',
			),*/
			/*array(
			...
			array(
				'name'               => 'bbPress',
				'slug'               => 'bbpress',
				'source'             => get_stylesheet_directory() . '/lib/plugins/bbpress.zip',
				'required'           => false,
				'version'           => '2.5.8',
			),
			array(
				'name'               => 'LayerSlider',
				'slug'               => 'LayerSlider',
				'source'             => get_stylesheet_directory() . '/lib/plugins/LayerSlider.zip',
				'required'           => false,
				'version'           => '5.6.2',
			),
...
if ( ! function_exists( 'spring_excerpt' ) ) :
/**
 ...
 * @return string
 */
function spring_excerpt( $characters ){
	//$characters = 70;
...
TMM::init();
if (TMM::get_option("avatar_cache")){ 
...
if (TMM::get_option("url_nofollow")){
...
function leaf_cdn_replace($html){
	...
	$leaf_host = 'http://img.wordpressleaf.com'; //七牛域名
