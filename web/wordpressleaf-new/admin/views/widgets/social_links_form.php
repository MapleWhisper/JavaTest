...
if (!empty($social_types)){
	...
	foreach ($social_types as $key => $type) {
		...
		<p>
			<label for="<?php echo esc_attr($widget->get_field_id($key . '_tooltip')); ?>"><?php echo $type . ' ' . esc_html__('提示文本', 'diplomat'); ?>:</label>
		...
		if ($key === 'rss') {
			...
			<p>
				...
				<label for="<?php echo esc_attr($widget->get_field_id($key . '_links')); ?>"><?php esc_html_e('显示RSS链接', 'diplomat') ?></label>
			</p>
			...
			<p>
				<label for="<?php echo esc_attr($widget->get_field_id($key . '_links')); ?>"><?php echo $type . ' ' . esc_html__('链接', 'diplomat'); ?>:</label>
