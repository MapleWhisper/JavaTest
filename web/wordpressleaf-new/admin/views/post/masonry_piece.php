						...
						<p>
							...
							if ( strpos( $post->post_content, '<!--more-->' ) ) {
								...
								if ( empty( $post->post_excerpt ) ) {
									...
									$txt = strip_tags( $txt );
									echo ( mb_strlen( $txt ) > $excerpt_symbols_count ) ? do_shortcode( mb_substr( $txt, 0, $excerpt_symbols_count ) . " ..." ) : do_shortcode( $txt );
								} else {
									echo ( mb_strlen( $post->post_excerpt ) > $excerpt_symbols_count ) ? do_shortcode( mb_substr( $post->post_excerpt, 0, $excerpt_symbols_count ) . " ..." ) : do_shortcode( $post->post_excerpt );
								}
